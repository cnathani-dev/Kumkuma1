...
export const EventCard: React.FC<EventCardProps> = ({ event, onEdit, onDelete, onDuplicate, onNavigate, canModify, canAccessFinances, showClientName = false, onClientClick, onStateChange, onRequestCancel }) => {
    ...
    const templateName = useMemo(() => {
        if (event.templateId === 'NO_FOOD') return 'No Food Event';
        if (!event.templateId) return 'Custom Menu';
        return templates.find(t => t.id === event.templateId)?.name || 'Unknown Template'
    ...
    return (
        <div className={`p-5 bg-white dark:bg-warm-gray-800 rounded-lg shadow-md ${isReadOnly ? 'opacity-70' : ''}`}>
            ...
            <div className="mt-4 pt-4 border-t border-warm-gray-200 dark:border-warm-gray-700 flex flex-col gap-4">
                <div className="flex flex-wrap gap-2 justify-start items-center">
                    {showMenuButtons && (
                        <>
                            <button onClick={() => onNavigate('MENU_CREATOR')} className={secondaryButton} disabled={!showMenuButtons}><FileEdit size={16}/> {isReadOnly ? 'View Menu' : 'Edit Menu'}</button>
                            <button onClick={() => onNavigate('LIVE_COUNTER_SELECTOR')} className={secondaryButton} disabled={!showMenuButtons}><Salad size={16}/> {isReadOnly ? 'View' : 'Edit'} Counters</button>
                            <button onClick={() => onNavigate('SERVICE_PLANNER')} className={secondaryButton} disabled={isReadOnly}><ConciergeBell size={16}/> Service Plan</button>
                            <button onClick={() => onNavigate('KITCHEN_PLAN')} className={secondaryButton}><ChefHat size={16}/> Kitchen Plan</button>
                        </>
                    )}
                    {canAccessFinances &&
                        <button onClick={() => onNavigate('FINANCE')} className={secondaryButton}><Banknote size={16}/> Finances</button>
                    }
                </div>
                {canModify && 
                 <div className="flex flex-wrap gap-2 justify-end items-center">
                    {event.state === 'lead' && (
                        <>
                            ...
                            <button onClick={() => onStateChange(event, 'lost')} className={iconButton('hover:bg-red-100 dark:hover:bg-red-800')} title="Mark as Lost">
